  ...
  <table-manage>
    <template #search>
      <el-form-item label="名称">
        <el-input placeholder="规则名称" v-model="pageParam.searchObject.name"/>
      </el-form-item>
      <el-form-item label="提供者">
        <el-select style="width: 180px" v-model="pageParam.searchObject.provider" clearable>
          <el-option label="OpenAI Moderation" value="openai_moderation"/>
          <el-option label="DashScope" value="dashscope"/>
          <el-option label="敏感词" value="sensitive_word"/>
          <el-option label="关键字正则" value="keyword_regex"/>
        </el-select>
      </el-form-item>
    ...
    <template #operate>
      <el-button :icon="Plus" type="primary" @click="onCreate">添加规则</el-button>
    ...
    <template #default>
      <el-table v-loading="loading" :data="pageVo.records" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="provider" width="180" label="提供者">
          <template #default="scope">
            <el-tag type="primary">{{ providerLabel(scope.row.provider) }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="action" width="100" label="动作">
          <template #default="scope">
            <el-tag :type="actionTag(scope.row.action)">{{ actionLabel(scope.row.action) }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="threshold" width="100" label="阈值"/>
    ...
    <template #form>
      <el-dialog v-model="dialogFormVisible" :title="`${editForm.id ? '编辑' : '添加'}审核规则`" width="680px">
        <el-form ref="formRef" label-width="140px" :model="editForm" :rules="rules">
          <el-form-item prop="name" label="规则名称">
            <el-input v-model="editForm.name" placeholder="如: OpenAI 内容审核"/>
          </el-form-item>
          <el-form-item prop="provider" label="提供者">
          ...
          <el-form-item label="配置 (JSON)">
            <el-input v-model="editForm.configJson" type="textarea" :rows="4"
                      placeholder='如: {"pattern": ".*(spam|ad).*"} 或 {"apiKey": "sk-..."}'/>
          </el-form-item>
          <el-form-item label="阈值">
            <el-input-number v-model="editForm.threshold" :min="0" :max="1" :step="0.05" :precision="2"/>
          </el-form-item>
          <el-form-item label="动作">
            <el-radio-group v-model="editForm.action">
              ...
              <el-radio label="pass">放行</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="作用位置">
...
import {listByPage, add, update, deleteBatch} from "@/api/ai/moderation-rule-api";
...
const editForm = ref<any>({enableStatus: 1, sort: 0, provider: 'keyword_regex', action: 'block', target: 'both'});

const rules = reactive({
  name: [{required: true, message: '请输入规则名称', trigger: 'blur'}],
  provider: [{required: true, message: '请选择提供者', trigger: 'change'}]
});
...
const actionLabel = (a: string) => ({block: '拦截', replace: '替换', warn: '警告', pass: '放行'} as any)[a] || a;
const actionTag = (a: string) => ({block: 'danger', replace: 'warning', warn: 'info', pass: 'success'} as any)[a] || 'info';
...
const loadTableData = () => {
  ...
  const param = {
    ...
    aiModerationRule: pageParam.value.searchObject
  };
...
const onCreate = () => {editForm.value = {enableStatus: 1, sort: 0, provider: 'keyword_regex', action: 'block', target: 'both'}; dialogFormVisible.value = true;}

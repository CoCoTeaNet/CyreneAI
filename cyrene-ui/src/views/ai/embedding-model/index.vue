  ...
  <table-manage>
    <template #search>
      ...
      <el-form-item label="提供商">
        <el-input placeholder="dashscope / openai" v-model="pageParam.searchObject.providerType" style="width: 200px"/>
      </el-form-item>
    ...
    <template #default>
      <el-table v-loading="loading" :data="pageVo.records" style="width: 100%">
        <el-table-column prop="providerType" width="120" label="提供商"/>
        <el-table-column prop="modelName" width="200" label="模型名称"/>
        <el-table-column prop="dimension" width="100" label="向量维度"/>
        <el-table-column prop="apiBaseUrl" width="250" label="API地址" show-overflow-tooltip/>
        <el-table-column prop="isDefault" width="80" label="默认">
        ...
        <el-table-column prop="enableStatus" width="80" label="状态">
          <template #default="scope">
        ...
        <el-table-column prop="remark" width="200" label="备注" show-overflow-tooltip/>
    ...
    <template #form>
      <el-dialog v-model="dialogFormVisible" :title="`${editForm.id ? '编辑' : '添加'}嵌入模型`" width="600px">
        <el-form ref="formRef" label-width="120px" :model="editForm" :rules="rules">
          <el-form-item prop="providerType" label="提供商">
            <el-select v-model="editForm.providerType" placeholder="选择提供商" style="width: 100%">
              <el-option label="DashScope" value="dashscope"/>
              <el-option label="OpenAI" value="openai"/>
            </el-select>
          </el-form-item>
          <el-form-item prop="modelName" label="模型名称">
            <el-input v-model="editForm.modelName" placeholder="例: text-embedding-v3"/>
          </el-form-item>
          <el-form-item label="API密钥">
            <el-input v-model="editForm.apiKey" type="password" show-password placeholder="留空使用全局默认密钥"/>
          </el-form-item>
          <el-form-item label="API地址">
            <el-input v-model="editForm.apiBaseUrl" placeholder="留空使用官方默认地址"/>
          </el-form-item>
          <el-form-item label="向量维度">
            <el-input-number v-model="editForm.dimension" :min="0" :step="256" style="width: 100%"/>
          </el-form-item>
...
import {listByPage, add, update, remove} from "@/api/ai/embedding-model-api";
...
const rules = reactive({
  providerType: [{required: true, message: '请选择提供商', trigger: 'change'}],
...
const loadTableData = () => {
  ...
  let param = {
    ...
    embeddingModel: pageParam.value.searchObject
  };
...
const onDelete = (id: string) => {
  ElMessageBox.confirm('确认删除该嵌入模型?', '提示', {

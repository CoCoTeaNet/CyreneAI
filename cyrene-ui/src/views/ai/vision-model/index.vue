  ...
  <table-manage>
    <template #search>
      ...
      <el-form-item label="提供商">
        <el-select placeholder="选择提供商" style="width: 200px" v-model="pageParam.searchObject.providerType" filterable clearable>
    ...
    <template #form>
      <el-dialog v-model="dialogFormVisible" :title="`${editForm.id ? '编辑' : '添加'}视觉模型`" width="600px">
        <el-form ref="formRef" label-width="120px" :model="editForm" :rules="rules">
          ...
          <el-form-item prop="modelName" label="模型名称">
            <el-input v-model="editForm.modelName" placeholder="例: gpt-4o"/>
          </el-form-item>
...
import {listByPage, add, update, deleteBatch} from "@/api/ai/vision-api";
...
const loadTableData = () => {
  ...
  let param = {
    ...
    visionModel: pageParam.value.searchObject
  };
...
const onDelete = (id: string) => {
  ElMessageBox.confirm('确认删除该视觉模型?', '提示', {
...
const onDeleteBatch = () => {
  ...
  ElMessageBox.confirm('确认删除所选视觉模型?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }
